...
def test_cost_estimate_configs_in_args_happy_case():
    result = runner.invoke(app, ['-h', 'http://10.20.31.32:8000', '-l', '/path/to/logfile', '-u', '100', '-r', '30', '-t', '1h20m', '-f', 'some_cortisol_file.py'])
...
def test_cost_estimate_one_missing_arg():
    result = runner.invoke(app, ['-h', 'http://10.20.31.32:8000', '-l', '/path/to/logfile', '-u', '100', '-r', '30', '-t', '1h20m'])
